...
function handleStatusTagClick(event) {
    ...
    if (originalStatus) {
        ...
        console.warn('Status tag clicked without original status:', event.target);
    }
}
...
/**
 * Gets status tag HTML with proper translation
 * @param {string} status - Hospital status
...
function getStatusTag(status, isActive = false) {
    ...
    // Utiliser la traduction ou le statut original si pas de traduction
    ...
    return `<span class="status-tag status-${statusClass}${activeClass}" 
                 ...
                 role="button" 
                 aria-pressed="${isActive}">${translatedStatus}</span>`;
}
